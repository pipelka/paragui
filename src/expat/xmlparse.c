	 ...
	 const char **nextPtr)
{
  ...
  for (;;) {
	...
	*eventPP = next;
	...
	poolFinish(&dtd.pool);
	if (dtd.complete && !defineAttribute(declElementType, declAttributeId, declAttributeIsCdata, 0, attVal))
	  return XML_ERROR_NO_MEMORY;
	break;
      }
	...
	if (declEntity) {
	  ...
	  poolFinish(&dtd.pool);
	  if (internalParsedEntityDeclHandler && ((ENTITY *)lookup(&dtd.generalEntities, declEntity->name, 0) == declEntity)) {
	    *eventEndPP = s;
